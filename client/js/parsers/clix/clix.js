...
export function transformARTtimeout(timeValue) {
  ...
  const timeValToSeconds = (hours * 3600) + (minutes * 60) + seconds;
  return timeValToSeconds;
}
  ...
  switch (item.json.genusTypeId) {
    ...
    case 'question-type%3Aqti-upload-interaction-audio%40ODL.MIT.EDU': {
      ...
      if (mapGenusType[item.json.genusTypeId]) {
        ...
        if (item.json.timeValue) {
          const timeValue = item.json.timeValue;
          audioTimeinSecs = transformARTtimeout(timeValue);
        }
      }

      return _.merge({}, qti,
        { title: item.title, question_type: questionType, audioTimeout: audioTimeinSecs });
    }
